                            ...
                            <div class="max-w-md mx-auto">
                                <div class="flex items-center space-x-5">
                                    <div class="block pl-2 font-semibold text-xl self-start text-gray-700">
                                        <h2 class="leading-relaxed">Preencha os campos</h2>
                                ...
                                <div class="divide-y divide-gray-200">
                                    <div
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            <input type="text"
                                                   class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                                                   placeholder="" id="endereco" wire:model="endereco">
                                            @error('endereco') <span class="text-red-500">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="flex flex-col">
                                            <label class="leading-loose">Data Nascimento*</label>
                                            <input type="date"
                                                   class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                                                   placeholder="" id="data_nasc" wire:model="data_nasc">
                                            @error('data_nasc') <span
                                                class="text-red-500">{{ $message }}</span>@enderror
                                        </div>
                                        @foreach($this->contatos as $index => $value)
                                            <div class="flex items-center space-x-4">
                                                <div class="flex flex-col">
                                                    <label class="leading-loose">Tipo</label>
                                                    <div class="relative focus-within:text-gray-600 text-gray-400">
                                                        <select class="px-8 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600s"
                                                                name="contatos.nome[{{$index}}]"
                                                                wire:model="contatos.{{$index}}.nome"
                                                        >
                                                            <option>-selecione-</option>
                                                            @foreach($tipo_contato as $tipo_contatos )
                                                                <option value="{{$tipo_contatos->id}}"> {{$tipo_contatos->nome}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="flex flex-col">
                                                    <label class="leading-loose">Valor</label>
                                                    <div class="relative focus-within:text-gray-600 text-gray-400">
                                                        <input type="text"
                                                               class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                                                               placeholder="" wire:model="contatos.{{$index}}.valor" name="contatos.valor[{{$index}}]">
                                                    </div>
                                                </div>
                                                <div class="flex flex-col">
                                                    <button wire:click.prevent="removeContato({{$index}})" type="button"
                                                            class="text-blue-800 text-sm mt-9 py-2 px-2">Delete
                                                    </button>
                                                </div>
                                            </div>
                                        @endforeach
                                        <div class="flex flex-col">
                                            <button wire:click.prevent="addContato" type="button"
                                                    class="focus:outline-none text-white text-sm mt-9 py-2.5 px-5 rounded-md bg-blue-500 hover:bg-blue-600 hover:shadow-lg">
                                                Adicionar Contato
                                            </button>
                                        </div>
                                    </div>
                                    <div class="pt-4 flex items-center space-x-4">
                                        <button wire:click.prevent="delete({{$pessoa_id}})" type="button"
                                        ...
                                        <button wire:click="closeModal()" type="button"
                                                ...
                                                style="background-color: rgb(225, 226, 220)">
